...
mod tests {
    ...
    fn test_csi_insert_lines() {
        let state = Arc::new(Mutex::new(test_state(10, 3)));
        let mut handler = test_handler(state.clone());
        // Write three lines: row0 "AAA", row1 "   BBB", row2 "      CCC"
        feed(&mut handler, "AAA\nBBB\nCCC");
        feed(&mut handler, "\x1b[1;1H");
        feed(&mut handler, "\x1b[1L");
        {
            let s = state.lock().unwrap();
            // Inserting at row0 shifts everything down: row0 blank, row1 old row0 ("AAA"), row2 old row1 ("   BBB")
            assert_row(&s, 0, "          ");
            assert_row(&s, 1, "AAA       ");
            assert_row(&s, 2, "   BBB    ");
